function getSwiperSelected(swiper: HTMLElement): string {
...
function setSwipperSelected(swiper: HTMLElement, input: string | null) {
    const column = swiper.getElementsByTagName("ol")[0];
    const children = column.getElementsByTagName("li")
...
function setElementText(elem: Element, text: string) {
...
function renderResult(result: Element, carry: Element, value: number[]) {
...
function formatCarry(input: string): string {
    return formatNumberString(input).replaceAll('0', '\xa0').replaceAll(',', '\xa0');
}

function getCarry(input: number[]): string {
